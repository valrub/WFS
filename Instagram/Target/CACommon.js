//-----------------------------------------------------------------------------------------------------//
///////////////////////////////////////////////////////////////////////////////////////////////////////
/////////////////////// (#) STANDARD LIBRARY  (002.002.028) ///////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////////////////
//-----------------------------------------------------------------------------------------------------//
/*
// Changed by Valentin 29/5/2018.
// Commented printing of:
    // 1) You used a debug level of 4 which is a valid debug level.
    // 2) setGlobalLogger STARTED with debugLevel = 4
...
var Logger = {
    ...
    //KNOWN CUSTOM ERRORS
    _customErrors: {
        '1_801': 'Configuration Not Found',
        '1_600': 'Wrong Input Params',
        '1_666': 'FATAL ERROR! PROCESS ENDED.',
        '1_400011': 'XPATH NOT FOUND',
        '1_204': 'No content for this part'
    },
    //-------------------------------------------------------------------------------------------------//
    _statisticsMessage: ""
        //-------------------------------------------------------------------------------------------------//
    ...
    production: function(pMessage, pErrCode) {
            this._log(pMessage, pErrCode, 'WARNING', '787', 1, false);
        }
        //-------------------------------------------------------------------------------------------------//
        ,
...
function setGlobalLogger(pRTE, pIE, pOE, pExecutor, debugLevel = 1) {
...
var PersistObject = {
    ...
    addEntity: function addEntity(entity) {
        var _self = this;
        entity.body = RemoveAdditionalContentFromBody(entity.body);
        entity.description = RemoveAdditionalContentFromBody(entity.description);
        try {
    ...
    addImage: function addImage(entity) {
        ...
        if (entity.imageUrl) {
            if ((entity.itemType == "4" && entity.imageUrl.match(/fbcdn/)) || (entity.itemType == "4" && entity.imageUrl.match(/graph\.facebook\.com/))) {
                entity.imageUrl = "https://graph.facebook.com/" + entity.externalId + "/picture?type=large&width=200&height=200";
            }
        }
        ...
        try {
            if (_self._persistSettings.downloadVideoFiles && entity.itemType == "22" && !_self._persistSettings.useSaveBinaryForVideos) {
                ...
                //Logger.production("vutre sum Video 666");
                _self._persistSettings.collectedRecords += 1;
                _self._updateStatistics(entity);
    ...
    onSuccess: function onSuccess(filePath, entity) {
        var _self = this;
        try {
            //Logger.production(entity.description);
    ...
    callStackRemove: function callStackRemove(fName) {
        var _self = this;
        _self._callStack = _self._callStack.filter(function(item) {
            return item !== fName;
        });
...
////// Remove "See More", "See Translation", "Show More", etc.. //////
function RemoveAdditionalContentFromBody(body) {
    var currentBody = body;
    String.isNullOrEmpty = function(value) {
        return (!value || value == undefined || value == "" || value.length == 0);
    }
    if (!String.isNullOrEmpty(currentBody) && !isNumeric(currentBody)) {
        var regex = "(" + additionalContent.join('|') + ")$";

        var expReg = new RegExp(regex, "gi");

        currentBody = currentBody.trim();

        while (expReg.test(currentBody)) {
            currentBody = currentBody.replace(expReg, '').trim();
        }
    }
    return currentBody;
}

function makeVMFRequestObject(pIE, pExecutor) {
    return function(inputObj) {
        ...
        var vmfInputObj = {
            input: {
                taskId: pIE.taskId,
                cuId: pIE.cuId,
                async: (pIE.taskId && pIE.cuId) ? true : false,
                agentname: pIE.username,
                agentpass: pIE.password,
                proxyIP: pIE.VA_ProxyIP,
                staticLibrariesHost: pExecutor.getConfigParam("ADMIN_CONNECTIVITY_SERVER_HOST") // for VMF v6
            },
            config: {} // for VMF v6
        };
        // Add extra input parameters
        Object.keys(inputObj.input).forEach(function (key) {
            if (!vmfInputObj.hasOwnProperty(key)) {
                vmfInputObj.input[key] = inputObj.input[key];
                vmfInputObj.config[key] = inputObj.input[key]; // <=== Support for VMF v6
            }
        });
        // Add targets, excluding empty ones
        vmfInputObj.targets = inputObj.targets.filter(function (target) {
            return Boolean(target);
        });
        return JSON.stringify(vmfInputObj);
    }
}

// VMF Request Builder
function makeVMFRequest(re, ie, oe, executor) {
    !window.Logger ? setGlobalLogger(re, ie, oe, executor) : '';
    return function(requestObject) {
        Logger.production("Starting VMF request function");
        var req = new XMLHttpRequest();
        var vmfIP = executor.getConfigParam("VMF_HOST_IP");
        vmfIP = vmfIP ? vmfIP.trim() : 'CU1';
        Logger.production("VMF IP: " + vmfIP);
        ...
        req.onload = function() {
            if (req.status === 200) {
                ...
                if (response.output) {
                    ...
                    if (data.length === 0) {
                        Logger.production('WF could not retrieve any data from service.')
                    } else {
                        Logger.production('Total entities: ' + data.length);
                        data.forEach(function (entity) {
                            addImage(entity);
                        });
                        Logger.production('WF has finished collection.');
                    }
                } else {
                    Logger.production('Task was received and will be processed ASAP!\n' + JSON.stringify(response.input));
                }
                finalize();
            } else if (req.status === 400) {
                var error = JSON.parse(req.response).error;
                Logger.failure('Bad request! Input parameteres for VMF request are rejected!\n' + JSON.stringify(error));
            } else {
                Logger.failure('Server Error. Server returned status code: ' + req.status + '\nPlease check VMF log!');
            }
        };

        req.onerror = function() {
            Logger.failure('Network Error on ' + requestObject.input.platform + ' request: ' + req.statusText +
                '\nPossible causes:\n1. Agent is with proxy and VMF cannot receive calls from outside network\n' +
                '2. Call to VMF was not made from "about:blank" page, and because of that is blocked by current page!');
        };

        var vmfRequestObject = makeVMFRequestObject(ie, executor)(requestObject);

        req.setRequestHeader("Content-Type", "application/json");
        Logger.production("sending: " + vmfRequestObject);
        req.send(vmfRequestObject);
        Logger.production("VMF request sent!");
    }
}
