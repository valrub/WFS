function main(re, ie, oe, executor) {
    try {
		...
		var t = new Date();
		Logger.production("Second CA started started: " + t);
		...
		var getLiveQueryResults = function (url, chunk, sequence) {
			try {
				var searchTerm = getSearchTerm(url);
    ...
    //             else if (responseObj[i].interaction.sub_type === "video" && ie.downloadVideoFiles) {
    //                 Logger.production('Video url is ' + post.url);
				// 	if(/youtube\.com/.test(post.url)){
    // 				    post.itemType = "22"; // Video
    // 				    addImage(post);
    //                     Logger.production('Youtube video with url ' + post.url + ' will be downloaded');
				// 	} 
				// } 
                else {
