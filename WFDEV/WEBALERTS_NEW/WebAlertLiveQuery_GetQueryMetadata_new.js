function main(re, ie, oe, executor) {
  try {
    ...
    if(rgx.test(inp)){
        ...
        Logger.error('Wrong Input. Should be like: [ https://webalert.verint.com/#/query-data/17753 ]', true);
        executor.ready()
    }  
    ...
    getLiveQueryResults(queryID,  ""); //COMMENT LATER

    //--------------------------------------------------------------------------------------------------------------------
    function getLiveQueryResults(QueryID, sequence) {
      try {
